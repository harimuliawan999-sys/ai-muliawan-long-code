...
Pour déboguer les problèmes avec AIMLC, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
AIMLC Desktop exécute un serveur AIMLC local (le side-car `opencode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `AIMLC` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
AIMLC peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que AIMLC puisse le reconstruire.

1. Quittez complètement AIMLC Desktop.
...
3. Redémarrez le bureau AIMLC.

---

### Résoudre les problèmes de connexion au serveur

AIMLC Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Sur Windows, AIMLC Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de AIMLC.

---

### Les notifications ne s'affichent pas

AIMLC Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour AIMLC dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau AIMLC.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application AIMLC Desktop) :
...
Si vous rencontrez des problèmes avec AIMLC :
