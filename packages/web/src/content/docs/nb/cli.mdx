---
title: CLI
description: AIMLC CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AIMLC CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
opencode
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med AIMLC programmatisk.
...
Start AIMLC-terminalbrukergrensesnittet.
...
AIMLC CLI har også følgende kommandoer.

---

### agent

Administrer agenter for AIMLC.
...
Koble en terminal til en allerede kjørende AIMLC-backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern AIMLC-backend. For eksempel:
...
AIMLC drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `opencode auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Når AIMLC starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i AIMLC.
...
Kjør AIMLC i ikke-interaktiv modus ved å sende en melding direkte.
...
| `--attach`   |      | Koble til en kjørende AIMLC-server (f.eks. http://localhost:4096)     |
...
Start en headless AIMLC-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
opencode serve
```

Dette starter en HTTP-server som gir API tilgang til AIMLC-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
Vis tokenbruk og kostnadsstatistikk for AIMLC-øktene dine.
...
Importer øktdata fra en JSON-fil eller en AIMLC share-URL.

```bash
opencode import <file>
```

Du kan importere fra en lokal fil eller en AIMLC share-URL.
...
Start en headless AIMLC-server med et webgrensesnitt.

```bash
opencode web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til AIMLC via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `opencode`).
...
Avinstaller AIMLC og fjern alle relaterte filer.
...
Oppdaterer AIMLC til siste versjon eller en spesifikk versjon.
...
AIMLC CLI bruker følgende globale flagg.
...
AIMLC kan konfigureres ved hjelp av miljøvariabler.
