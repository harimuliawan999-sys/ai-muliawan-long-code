---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di AIMLC.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AIMLC fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire AIMLC avvia la TUI per la directory corrente.
...
Quando usi la TUI di AIMLC, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a AIMLC. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
