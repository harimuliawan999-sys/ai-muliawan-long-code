...
AIMLC Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

:::note
AIMLC Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in AIMLC. Ti abboni a AIMLC Go e
...
AIMLC Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

AIMLC Go funziona come qualsiasi altro provider in AIMLC.

1. Accedi a **<a href={console}>AIMLC Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `AIMLC Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a AIMLC Go.
:::
...
AIMLC Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua AIMLC config
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con AIMLC.
