...
In AIMLC esistono due tipi di agenti: agenti primari e subagenti.
...
AIMLC include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
AIMLC include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

AIMLC include due agenti primari integrati e due subagenti integrati.
...
Se non specifichi una temperature, AIMLC usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di AIMLC sia per la config specifica del progetto.
...
L'ID modello nella configurazione AIMLC usa il formato `provider/model-id`. Per esempio, se stai usando [AIMLC Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/harimuliawan999-sys/ai-muliawan-long-code).
:::
