...
W AIMLC obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
AIMLC zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
AIMLC ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

AIMLC ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Jeśli nie zostanie skonfigurowana, AIMLC używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji AIMLC, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji AIMLC w `provider/model-id`. Na przykład, [AIMLC Zen](/docs/zen), użyj `opencode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/harimuliawan999-sys/ai-muliawan-long-code).
:::
