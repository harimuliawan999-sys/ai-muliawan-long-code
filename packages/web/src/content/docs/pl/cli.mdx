---
title: CLI
description: Opcje i polecenia wiersza poleceń AIMLC.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI AIMLC domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
opencode
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać AIMLC programowo.
...
Uruchamia interfejs terminalowy (TUI) AIMLC.
...
Interfejs CLI AIMLC zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami AIMLC.
...
Dołącz terminal do już działającego serwera backendu AIMLC uruchomionego za pomocą `serve` lub `web`.

```bash
opencode attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem AIMLC. Na przykład:
...
AIMLC integruje się z wieloma dostawcami. Możesz użyć `opencode auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Podczas uruchamiania AIMLC ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w AIMLC.
...
Uruchom AIMLC w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
| `--attach`   |       | Dołącz do działającego serwera AIMLC (np. http://localhost:4096)     |
...
Uruchom serwer AIMLC (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
opencode serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API AIMLC bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Pokaż statystyki wykorzystania tokenów i sesji AIMLC.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania AIMLC.
...
Uruchom serwer AIMLC z interfejsem internetowym.

```bash
opencode web
```

Uruchamia to serwer HTTP i udostępnia AIMLC przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `opencode`).
...
Odinstaluj AIMLC i usuń wszystkie powiązane pliki.
...
Aktualizuj AIMLC do najnowszej lub określonej wersji.
...
Interfejs CLI AIMLC przyjmuje następujące flagi globalne dla każdego polecenia.
...
AIMLC można skonfigurować za pomocą zmiennych środowiskowych.
