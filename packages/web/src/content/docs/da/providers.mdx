---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i AIMLC.
---

import config from "../../../../config.mjs"
export const console = config.console

AIMLC bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i AIMLC-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i AIMLC-konfigurationen.
...
AIMLC Zen er en liste over modeller leveret af AIMLC-teamet, som er blevet testet og verificeret til at fungere godt med AIMLC. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med AIMLC Zen.
:::
...
Det fungerer som alle andre udbydere i AIMLC og er helt valgfrit at bruge.

---

## AIMLC Go

AIMLC Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af AIMLC-teamet, som er testet og verificeret til at fungere godt med AIMLC.

1. Kør kommandoen `/connect` i TUI, vælg `AIMLC Go`, og gå til [aimuliawan.dev/auth](https://aimuliawan.dev/zen).
...
Sådan bruger du Amazon Bedrock med AIMLC:
...
:::info
At bruge dit Claude Pro/Max-abonnement i AIMLC understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
AIMLC bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse AIMLC
...
Sådan bruger du GitHub Copilot-abonnementet med AIMLC:
...
Sådan bruger du Google Vertex AI med AIMLC:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver AIMLC-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver AIMLC-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for AIMLC. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med AIMLC:
...
3. Kopier API-nøglen til brug i AIMLC.
...
6. **Vigtigt**: Før du bruger skymodeller i AIMLC, skal du hente modelinformationen lokalt:
...
AIMLC Zen er en liste over testede og verificerede modeller leveret af AIMLC-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>AIMLC Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **AIMLC Zen**.

   ```txt
   /connect
   ```

3. Indtast AIMLC API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med AIMLC:
...
`limit`-felterne lader AIMLC forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
