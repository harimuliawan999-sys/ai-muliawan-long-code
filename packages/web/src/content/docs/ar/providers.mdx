---
title: المزوّدون
description: استخدام أي موفّر LLM في AIMLC.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم AIMLC ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات AIMLC.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات AIMLC.
...
AIMLC Zen هي قائمة نماذج يوفّرها فريق AIMLC وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع AIMLC. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع AIMLC Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opencode، ثم انتقل إلى [aimuliawan.dev/auth](https://aimuliawan.dev/auth).
...
يعمل مثل أي مزوّد آخر في AIMLC واستخدامه اختياري بالكامل.

---

## AIMLC Go

AIMLC Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق AIMLC، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع AIMLC.

1. شغّل الأمر `/connect` في TUI، واختر `AIMLC Go`، ثم انتقل إلى [aimuliawan.dev/auth](https://aimuliawan.dev/zen).
...
لاستخدام Amazon Bedrock مع AIMLC:
...
:::info
استخدام اشتراك Claude Pro/Max داخل AIMLC غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم AIMLC نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر AIMLC
...
لاستخدام Google Vertex AI مع AIMLC:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة AIMLC تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة AIMLC مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع AIMLC. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع AIMLC:
...
3. انسخ مفتاح API لاستخدامه في AIMLC.
...
6. **مهم**: قبل استخدام نماذج السحابة في AIMLC، يجب سحب معلومات النموذج محليا:
...
AIMLC Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق AIMLC. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>AIMLC Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **AIMLC Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ AIMLC.
...
تسمح حقول `limit` لـ AIMLC بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
