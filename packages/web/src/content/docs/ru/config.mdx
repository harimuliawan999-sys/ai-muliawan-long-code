...
Например, если ваша глобальная конфигурация устанавливает `theme: "aimlc"` и `autoupdate: true`, а конфигурация вашего проекта устанавливает `model: "anthropic/claude-sonnet-4-5"`, окончательная конфигурация будет включать все три параметра.
...
Файл конфигурации имеет схему, определенную в [**`aimuliawan.dev/config.json`**](https://aimuliawan.dev/config.json).

Конфигурация TUI использует [**`aimuliawan.dev/tui.json`**](https://aimuliawan.dev/tui.json).
