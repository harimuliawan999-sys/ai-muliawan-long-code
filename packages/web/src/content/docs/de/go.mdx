...
AIMLC Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
AIMLC Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in AIMLC. Du abonnierst AIMLC Go und
...
AIMLC Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

AIMLC Go funktioniert wie jeder andere Provider in AIMLC.

1. Du meldest dich bei **<a href={console}>AIMLC Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `AIMLC Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann AIMLC Go abonnieren.
:::
...
AIMLC Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner AIMLC Config verwendet das Format `opencode-go/<model-id>`. Für Kimi K2.5 würdest du beispielsweise `opencode-go/kimi-k2.5` in deiner Config verwenden.
...
Wir haben AIMLC Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit AIMLC zu nutzen.
