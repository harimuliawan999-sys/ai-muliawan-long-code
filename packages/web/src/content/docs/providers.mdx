---
title: Providers
description: Using any LLM provider in AIMLC.
---

import config from "../../../config.mjs"
export const console = config.console

AIMLC uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your AIMLC config.
...
You can customize the providers through the `provider` section in your AIMLC
config.
...
AIMLC Zen is a list of models provided by the AIMLC team that have been
tested and verified to work well with AIMLC. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with AIMLC Zen.
:::

1. Run the `/connect` command in the TUI, select `AIMLC Zen`, and head to [aimuliawan.dev/auth](https://aimuliawan.dev/zen).
...
It works like any other provider in AIMLC and is completely optional to use.

---

## AIMLC Go

AIMLC Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the AIMLC team that have been
tested and verified to work well with AIMLC.

1. Run the `/connect` command in the TUI, select `AIMLC Go`, and head to [aimuliawan.dev/auth](https://aimuliawan.dev/zen).
...
To use Amazon Bedrock with AIMLC:
...
AIMLC. Anthropic explicitly prohibits this.

Previous versions of AIMLC came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in AIMLC with zero setup:
...
GitLab Duo support in AIMLC is experimental. Features, configuration, and
behavior may change in future releases.
:::

AIMLC integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
AIMLC uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock AIMLC
...
When a `duo-workflow-*` model is selected, AIMLC will:
...
4. Route tool execution requests through AIMLC's permission-gated tool system
...
To use Google Vertex AI with AIMLC:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each AIMLC conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each AIMLC conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for AIMLC. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with AIMLC:
...
3. Copy the API key for use in AIMLC.
...
6. **Important**: Before using cloud models in AIMLC, you must pull the model information locally:
...
AIMLC Zen is a list of tested and verified models provided by the AIMLC team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>AIMLC Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **AIMLC Zen**.

   ```txt
   /connect
   ```

3. Enter your AIMLC API key.
...
The `limit` fields allow AIMLC to understand how much context you have left. Standard providers pull these from models.dev automatically.
