---
title: Proveedores
description: Usando cualquier proveedor LLM en AIMLC.
---

import config from "../../../../config.mjs"
export const console = config.console

AIMLC usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración AIMLC.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu AIMLC
configuración.
...
AIMLC Zen es una lista de modelos proporcionados por el equipo AIMLC que han sido
probado y verificado para funcionar bien con AIMLC. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con AIMLC Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione opencode y diríjase a [aimuliawan.dev/auth](https://aimuliawan.dev/auth).
...
Funciona como cualquier otro proveedor en AIMLC y su uso es completamente opcional.

---

## AIMLC Go

AIMLC Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de AIMLC que han sido
probado y verificado para funcionar bien con AIMLC.

1. Ejecute el comando `/connect` en TUI, seleccione `AIMLC Go` y diríjase a [aimuliawan.dev/auth](https://aimuliawan.dev/zen).
...
Para usar Amazon Bedrock con AIMLC:
...
:::info
El uso de su suscripción Claude Pro/Max en AIMLC no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
AIMLC utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear AIMLC
...
Para utilizar Google Vertex AI con AIMLC:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación AIMLC como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación AIMLC enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para AIMLC. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con AIMLC:
...
3. Copie la clave API para usarla en AIMLC.
...
6. **Importante**: Antes de usar modelos en la nube en AIMLC, debe obtener la información del modelo localmente:
...
AIMLC Zen es una lista de modelos probados y verificados proporcionada por el equipo AIMLC. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>AIMLC Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **AIMLC Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave AIMLC API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con AIMLC:
...
Los campos `limit` le permiten a AIMLC comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
