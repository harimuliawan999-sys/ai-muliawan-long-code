---
title: Plugins
description: Write your own plugins to extend AIMLC.
---

Plugins allow you to extend AIMLC by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify AIMLC's default behavior.
...
AIMLC runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
:::note
If you’re using the AIMLC desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://aimuliawan.dev/docs/sdk) for details.
