...
Örneğin, genel ayarınız `theme: "aimlc"` ve `autoupdate: true`'yi ayarlıyorsa ve proje ayarlarınız `model: "anthropic/claude-sonnet-4-5"`'yi ayarlıyorsa, nihai yapılandırma üç ayarı da içerecektir.
...
Yapılandırma dosyası [**`aimuliawan.dev/config.json`**](https://aimuliawan.dev/config.json)'da tanımlanan bir şemaya sahiptir.
