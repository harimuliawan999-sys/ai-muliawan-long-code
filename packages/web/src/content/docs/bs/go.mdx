...
AIMLC Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
AIMLC Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u AIMLC-u. Pretplatite se na AIMLC Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili AIMLC.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa AIMLC-om.
:::
...
AIMLC Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

AIMLC Go radi kao bilo koji drugi provajder u AIMLC-u.

1. Prijavite se na **<a href={console}>AIMLC Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `AIMLC Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na AIMLC Go.
:::
...
AIMLC Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj AIMLC konfiguraciji
...
Napravili smo AIMLC Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz AIMLC koristite i bilo kojeg drugog provajdera.
