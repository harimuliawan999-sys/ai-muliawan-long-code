...
Da biste otklonili probleme s AIMLC, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
AIMLC pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
AIMLC Desktop pokreće lokalni AIMLC server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `AIMLC` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
AIMLC također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga AIMLC mogao ponovo izgraditi.

1. Potpuno zatvorite AIMLC Desktop.
...
3. Ponovo pokrenite AIMLC Desktop.

---

### Rješavanje problema sa vezom na serveru

AIMLC Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Na Windows-u, AIMLC Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa AIMLC karakteristikama.

---

### Obavještenja se ne prikazuju

AIMLC Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za AIMLC u postavkama vašeg OS-a, i
...
1. Zatvorite AIMLC Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka AIMLC Desktop aplikacije):
...
Ako imate problema s AIMLC:

1. **Prijavite probleme na GitHub**

   Najbolji način da prijavite greške ili zatražite funkcije je putem našeg GitHub spremišta:

   [**github.com/harimuliawan999-sys/ai-muliawan-long-code/issues**](https://github.com/harimuliawan999-sys/ai-muliawan-long-code/issues)
...
2. **Pridružite se našem Discordu**
   ...
   [**aimuliawan.dev/discord**](https://aimuliawan.dev/discord)
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. AIMLC dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite AIMLC da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti AIMLC da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
AIMLC će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
