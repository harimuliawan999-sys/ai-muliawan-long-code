...
Možete dodati vanjske alate u AIMLC koristeći _Model Context Protocol_, ili MCP. AIMLC podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [AIMLC Config](https://aimuliawan.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
AIMLC automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, AIMLC će:
...
Ako server zahtijeva autentifikaciju, AIMLC će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, AIMLC sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u AIMLC, zajedno s ugrađenim alatima. Možete njima upravljati kroz AIMLC konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete AIMLC sa vašim Sentry nalogom.
