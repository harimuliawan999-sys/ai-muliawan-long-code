---
title: CLI
description: AIMLC CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AIMLC CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
opencode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa AIMLC.
...
Pokrenite AIMLC terminalski korisnički interfejs.
...
AIMLC CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte AIMLC agentima.
...
Priključite terminal na već pokrenut AIMLC backend server pokrenut putem `serve` ili `web` naredbi.

```bash
opencode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim AIMLC backend-om. Na primjer:
...
AIMLC pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `opencode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Kada se AIMLC pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u AIMLC.
...
Pokrenite AIMLC u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
| `--attach`   |        | Priključite na pokrenuti AIMLC server (npr. http://localhost:4096)    |
...
Pokrenite AIMLC headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
opencode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti AIMLC-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše AIMLC sesije.
...
Uvezite podatke sesije iz JSON datoteke ili AIMLC dijeljenog URL-a.

```bash
opencode import <file>
```

Možete uvesti iz lokalne datoteke ili AIMLC dijeljenog URL-a.
...
Pokrenite AIMLC headless server sa web interfejsom.

```bash
opencode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup AIMLC-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Deinstalirajte AIMLC i uklonite sve povezane datoteke.
...
Ažurira AIMLC na najnoviju verziju ili određenu verziju.
...
AIMLC CLI prihvata sljedeće globalne zastavice.
...
AIMLC se može konfigurirati pomoću varijabli okruženja.
