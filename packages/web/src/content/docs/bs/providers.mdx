---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u AIMLC.
---

import config from "../../../../config.mjs"
export const console = config.console

AIMLC koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj AIMLC konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem AIMLC
config.
...
AIMLC Zen je lista modela koje je obezbedio AIMLC tim koji su bili
testirano i potvrđeno da dobro radi sa AIMLC. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa AIMLC Zen.
:::
...
Radi kao i svaki drugi provajder u AIMLC i potpuno je opcionalan za korištenje.

---

## AIMLC Go

AIMLC Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža AIMLC tim i koji su testirani i verificirani da dobro rade s AIMLC-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `AIMLC Go` i idite na [aimuliawan.dev/auth](https://aimuliawan.dev/zen).
...
Da biste koristili Amazon Bedrock s AIMLC:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u AIMLC.
:::
...
AIMLC koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate AIMLC
...
Za korištenje Google Vertex AI s AIMLC:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki AIMLC razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki AIMLC razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za AIMLC. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s AIMLC:
...
3. Kopirajte API ključ za korištenje u AIMLC.
...
6. **Važno**: Prije upotrebe modela oblaka u AIMLC, morate lokalno povući informacije o modelu:
...
AIMLC Zen je lista testiranih i verifikovanih modela koju je obezbedio AIMLC tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>AIMLC Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **AIMLC Zen**.

```txt
   /connect
```

3. Unesite svoj AIMLC API ključ.
...
Polja `limit` omogućavaju AIMLC da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite AIMLC konfiguraciju i:
