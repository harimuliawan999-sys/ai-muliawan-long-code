...
import { NamedError } from "@aimuliawan/shared/util/error"
...
import { AppFileSystem } from "@aimuliawan/shared/filesystem"
import { InstanceState } from "@/effect"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { EffectFlock } from "@aimuliawan/shared/util/effect-flock"
...
const InfoSchema = Schema.Struct({
  ...
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://aimuliawan.dev/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://aimuliawan.dev/docs/agents" }),
...
function globalConfigFile() {
  const candidates = ["aimlc.jsonc", "aimlc.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://aimuliawan.dev/config.json"
        const updated = text.replace(/^\s*\{/, '{\n  "$schema": "https://aimuliawan.dev/config.json",')
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "aimlc.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "aimlc.jsonc"))),
      )
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://aimuliawan.dev/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://aimuliawan.dev/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("aimlc", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.AIMLC_CONFIG_DIR) {
          log.debug("loading config from AIMLC_CONFIG_DIR", { path: Flag.AIMLC_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".aimlc") || dir === Flag.AIMLC_CONFIG_DIR) {
            for (const file of ["aimlc.json", "aimlc.jsonc"]) {
          ...
          // Auto-discovered plugins under `.aimlc/plugin(s)` are already local files, so ConfigPlugin.load
