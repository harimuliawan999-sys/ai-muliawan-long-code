...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "aimlc" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
      ...
      flatMap((provider) =>
        pipe(
          ...
          map(([model, info]) => ({
            ...
            disabled: provider.id === "aimlc" && model.includes("-nano"),
            footer: info.cost?.input === 0 && provider.id === "aimlc" ? "Free" : undefined,
            onSelect() {
