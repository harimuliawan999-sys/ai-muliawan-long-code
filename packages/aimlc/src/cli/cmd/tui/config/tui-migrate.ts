...
const TUI_SCHEMA_URL = "https://aimuliawan.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from aimlc.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "aimlc"),
    ...(await Filesystem.findUp(["aimlc.json", "aimlc.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "aimlc"))
  }
