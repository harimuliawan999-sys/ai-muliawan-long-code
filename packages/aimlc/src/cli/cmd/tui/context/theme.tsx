...
import { Glob } from "@aimuliawan/shared/util/glob"
...
import opencode from "./theme/aimlc.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@aimuliawan/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "aimlc")
        draft.active = typeof active === "string" ? active : "aimlc"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.aimlc, store.mode)
    })
