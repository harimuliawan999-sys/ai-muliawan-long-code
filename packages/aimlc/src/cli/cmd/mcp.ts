    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in aimlc.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .aimlc/ subdirectory too)
  const candidates = [path.join(baseDir, "aimlc.json"), path.join(baseDir, "aimlc.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".aimlc", "aimlc.json"), path.join(baseDir, ".aimlc", "aimlc.jsonc"))
  }
  ...
  // Default to aimlc.json if none exist
  return candidates[0]
}
