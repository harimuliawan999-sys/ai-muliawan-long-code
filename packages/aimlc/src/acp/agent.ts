...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@aimuliawan/sdk/v2"
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // AIMLC stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const opencodeProvider = providers.find((p) => p.id === "aimlc")
  if (opencodeProvider) {
    if (opencodeProvider.models["big-pickle"]) {
      return { providerID: ProviderID.aimlc, modelID: ModelID.make("big-pickle") }
    }
