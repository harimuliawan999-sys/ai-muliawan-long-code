...
- Author package entrypoint is `@aimuliawan/plugin/tui`.
...
```json
{
  "$schema": "https://aimuliawan.dev/tui.json",
  "theme": "smoke-theme",
...
- Import types from `@aimuliawan/plugin/tui`.
- `@aimuliawan/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@aimuliawan/plugin/tui"
...
- The value is a semver range checked against the running AIMLC version.
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, AIMLC installs `@aimuliawan/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@aimuliawan/plugin/tui`.
